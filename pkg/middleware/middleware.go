...
func GetContextHandler() macaron.Handler {
	return func(c *macaron.Context) {
      ...
      ProcessUser:  &m.ProcessUser{},
      SubProcessUser:&m.SubProcessUser{},
      MachineUser:&m.MachineUser{},
			Session:        GetSession(),
			IsSignedIn:     false,
		...
		ctx.Logger = log.New("context", "userId", ctx.UserId, "orgId", ctx.SignedInUser.OrgId, "uname", ctx.Login)
		ctx.Data["ctx"] = ctx
...
func initContextWithAnonymousUser(ctx *Context) bool {
    ...
    ctx.SignedInUser.OrgId = orgQuery.Result.Id
...
func initContextWithApiKey(ctx *Context) bool {
    ...
    ctx.SignedInUser.OrgId= apikey.OrgId
		return true
	}
}
